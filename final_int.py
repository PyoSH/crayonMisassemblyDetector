...
def prob2(img,TL, BR, colorFlag): #이미지 하나에 대해서 검사하기 때문에 0 또는 1 리턴
    # 0. 손잡이 제외한 색상으로 이진화 
    img_bin = handle_bin(img)
    
    img_bin = cv2.bitwise_not(img_bin)
    
    # cv2.imshow('!handlebin',img_bin)
    
    # 1. 영상적분 
    img_int = cv2.integral(img_bin.copy())
    ...
    # print(img_bin.shape)
    sum_ROI = img_int[y2+1,x2+1] - img_int[y1,x2+1] - img_int[y2+1,x1] + img_int[y1,x1]
    print(sum_ROI)
    
    # 3. 임계값 넘는지 확인 & 임계값 넘으면 정상(1), 안넘으면 비틀림(0) turn = 0
    if colorFlag == 6 or colorFlag == 9 or colorFlag == 10 or colorFlag == 11 or colorFlag ==12: # 어두운 친구들 한정 
        # print(':', colorFlag)
        if 1500000 < sum_ROI < 100000000:
            turn =1
        else:
            turn =0
        ...
        # print(colorFlag)
        if 2450000 < sum_ROI < 10000000:
            turn =1
        elif 1000 < sum_ROI < 2450000:
            turn =0
...
def for_prob2(img, TL, TR,BL, BR, isFliped, order):
    ...
    # i=4
        ...
        singleWarp_BGR= cv2.cvtColor(singleWarp_dst.copy(),cv2.COLOR_HSV2BGR)
        isTurn[i] = prob2(singleWarp_dst, int_TL, int_BR, order[i])
...
def func_viz(img, TL, TR,BL, BR, isFliped, isTurn, isOrder, estOrder, answer):
    ...
    # i=4
        ...
        if isFliped[i] == 0 or isTurn[i] == 0 or estOrder[i] != answer[i]:
...
#         colorsH = img_pointChk[y,x,0]
#         colorsS = img_pointChk[y,x,1]
#         colorsV = img_pointChk[y,x,2]
#         colors = img_pointChk[y,x]
#         # print("Hue: ",colorsH)
...
#=이미지 획득========================================================================================================
filename = "../pics/img_023.jpg" # 입력 이미지 파일명을 적으세요.
